...
## Wave 183: config parsing rejects invalid UTF-8 (UBSan hardening)
Ran the ASan + UBSan unit suites this session (the "harden" axis). ASan: 100%
clean (3181 tests, no memory errors). UBSan: clean EXCEPT one finding — toml++
(third-party) reaches a __builtin_unreachable (UB) in unicode_autogenerated.hpp on
certain malformed input fed via Config::load_string. The TOML spec requires UTF-8,
so added infra::config::is_valid_utf8 (RFC 3629: rejects overlong/surrogate/>U+10FFFF)
and made Config::load_string reject invalid UTF-8 before toml::parse — closing the
invalid-UTF-8 UB class for all three configs (server/d2cs/d2dbs). New test in
test_config.cpp. GCC 14 build: only 2 -Wfree-nonheap-object FALSE POSITIVES
(d2dbs push_u16le, build_chat_event — both already reserve(); known GCC FP class,
build still succeeds).
DEFERRED (third-party, pre-existing, NOT network-reachable): the
toml_validator_property_test "near-valid TOML garbage" case still trips a toml++ UB
on VALID-UTF-8 malformed TOML (the generator's tokens are valid UTF-8: "é", NUL,
