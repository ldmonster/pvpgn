...
You will need to locate the file bnetd.toml and edit it (TOML comments use 
# at the start of a line; the legacy bnetd.conf format is no longer parsed
by the v3 binary -- see docs/toml-migration.md if you are upgrading).
Find the `storage_path =` line under `[files]` (or `[storage]`) and edit
it to use your chosen storage type as in the examples below:
...
A: Somehow you did not configured in bnetd.toml to use your chosen driver . 
