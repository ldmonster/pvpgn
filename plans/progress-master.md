...
Last updated: 2026-05-27 (Round 197)
...
- [ ] Step 11: Final integration and build verification
  ...
  - [x] R184.d: driver owns the init rejection path end-to-end. (1) `init_packet_dispatch_link.cpp`: on `pump_outcome != kAccepted` (unknown cclass / rate-limit hit / D2CS_BNETD with denied IP), `handle_init_packet` now returns `-1` immediately WITHOUT calling `pvpgn_v3_init_conn_apply_ex`. Both paths consult `dispatch_init_conn` against identical policy inputs so the bridge would just return -1 with no side effects -- skipping it shrinks the hot path and is the first concrete cut at legacy-bridge responsibilities. Emits one `eventlog_level_info` line with the cclass byte and `FeedOutcome` name on every reject. (2) Accept path still calls the bridge for its side effects (`conn_set_state` / `conn_set_class` / `handle_d2cs_init` / realmlist hookup) but the parity check on the accept path is now inverted: it only fires when driver accepted but the bridge then rejected, meaning "side-effect failure" (e.g. `handle_d2cs_init` returned -1) rather than verdict drift. (3) R185+ moves accept-path side effects out of `apply_via_legacy` so the bridge can be retired entirely. R184.a (findadreq), R184.b (anongame_lobby linked), R184.c (d2cs handle_init -- WITH_D2CS=OFF blocked) DEFERRED to R185. Docker verified: v3-test `pvpgn-v3-test:r184` sha256:5d4d3fb3c01de53d4827afff2acd9e40e98cb22d3404da9f89b64119037b08e6 (174 "All tests passed", identical to R183); v3-runtime `pvpgn-v3-runtime:r184` sha256:30a4e0e1e3c3a9ebb679a93289e59ef44119d79971f991be27040a65afa4a87d.
  - [x] R185.e: remove dead realmlist re-check from `apply_via_legacy`. (1) `init_conn_bridge_link.cpp`: the `realmlist_find_realm_by_ip` guard inside the `kD2csBnetd` branch was dead code as of R184.d (driver already gates via `kD2csIpDenied`). Deleted the duplicate check + its eventlog; accept path now does just `conn_set_state` + `conn_set_class` + `handle_d2cs_init`. (2) Audit: the 1-arg C ABI `pvpgn_v3_init_conn_apply` (which defaults `d2cs_ip_allowed=1` and would have relied on the in-handler check) has no callers in `src/` beyond the bridge itself; tests reach the handler through `FakeApply` not the linked impl. R185.a (InitSideEffects port refactor), R185.b (findadreq), R185.c (anongame_lobby linked), R185.d (WITH_D2CS=ON survey) DEFERRED to R186. Docker verified: v3-test `pvpgn-v3-test:r185` sha256:f97ba3a73b9bfd1b79dbabd225aae8e821a18359d985a7a1b7376dd19074b33c (174 "All tests passed", identical to R184); v3-runtime `pvpgn-v3-runtime:r185` sha256:c5d6f552ec4cffef9f02874813fa64e396e1fce03d03f4954c24af4819ed2860.
  - [x] R186.a: introduce `InitSideEffects` port + `feed_with_side_effects` driver method -- driver now end-to-end authoritative for init verdict AND side effects. (1) NEW `application/bnet_packet_pump/init_side_effects.hpp`: pure-C++ port with 4 nullable thunks (`set_connected`, `set_class`, `apply_d2cs_init`, `log_accept`) -- nulls are no-ops. (2) `driver.hpp`: new `feed_with_side_effects(span, PumpPolicy, void* conn, InitSideEffects const&) noexcept` (not constexpr -- fn-ptr dispatch isn't constexpr); delegates to `feed`, runs `log_accept -> set_connected -> set_class` on accept, plus `apply_d2cs_init` for `kD2csBnetd`; failure flips to `kRejected`. (3) NEW `integration/legacy_bnetd/init_side_effects_link.{hpp,cpp}`: linked-half singleton wiring the thunks into `bnetd_legacy` (`conn_set_state`, `conn_set_class`, `handle_d2cs_init`, `eventlog`). (4) `init_packet_dispatch_link.cpp` REWRITTEN bridge-free: handle_init_packet now just calls `driver.feed_with_side_effects(...)` -- the call to `pvpgn_v3_init_conn_apply_ex` is GONE. (5) `CMakeLists.txt`: added `init_side_effects_link.cpp` to `integration_legacy_bnetd_linked` SOURCES. (6) 6 new `[sx]` tests in `driver_test.cpp` covering accept paths, d2cs init failure -> kRejected, all-null table no-op, and rejection paths not invoking side effects. `apply_via_legacy` / `pvpgn_v3_init_conn_apply_ex` still link (registered at startup) but are no longer reached on the init path -- retirement is R187.a. R186.b (findadreq), R186.c (anongame_lobby linked) DEFERRED to R187. Docker verified: v3-test `pvpgn-v3-test:r186` sha256:36dc7fcf040a58d5f71649a8ac180f91a9511be519ea137d72afbec84efa7882 (174 "All tests passed" lines); v3-runtime `pvpgn-v3-runtime:r186` sha256:61dd7da16bd2065ecdaabd70719b0bcbdda95a09026f1de090feca8bb1940859.
  - [x] R186.e: prune unused legacy headers from `init_conn_bridge_link.cpp`. Removed `common/addr.h` and `bnetd/realm.h` (orphaned after R185.e removed the realmlist re-check); refreshed header comment to note this TU is now a dormant fallback. Pure cleanup -- verified by the same r186 images as R186.a.
  - [x] R187.a: collapse `apply_via_legacy` to delegate via the `InitSideEffects` port. (1) `init_conn_bridge_link.cpp` rewritten: per-class switch replaced by `dispatch_init_conn` -> `decision_to_class` -> shared `get_legacy_init_side_effects()` invocation chain (`log_accept` -> `set_connected` -> `set_class` -> optional `apply_d2cs_init`), shrinking the file from ~140 to ~70 LOC and removing the duplicated state/class transitions. Both the v3 driver path AND the legacy server.cpp pump path (`bnetd/handle_init.cpp` -> `pvpgn_v3_init_conn_apply_ex`) now share ONE source of truth for accept-path side effects. (2) Scope note: `pvpgn_v3_init_conn_apply_ex` and `install_legacy_init_conn_apply_handler` cannot be retired yet -- `bnetd/handle_init.cpp` (compiled when `WITH_BNETD=ON`) is still the only caller, and the v3 docker pipeline (`WITH_BNETD=OFF`) doesn't exercise that path. Repointing `handle_init.cpp` to `feed_with_side_effects` is queued for a future round. (3) R187.e audit conclusion: `PVPGN_V3_BNETD_INTEGRATION` is still load-bearing for `anongame_infos.cpp` (12+ gates) + `legacy_bridge.{hpp,cpp}` -- can't retire until those subsystems are bridge-free. R187.b (findadreq), R187.c (anongame_lobby linked), R187.d (WITH_D2CS=ON survey) DEFERRED to R188. Docker verified: v3-test `pvpgn-v3-test:r187` sha256:03c55f3c2d23bf743bc26b48acf45321eeb3bd696660a314a4eb6e03202a9c93 (174 "All tests passed", identical to R186); v3-runtime `pvpgn-v3-runtime:r187` sha256:e0069e0749c9ef761d0d2fe0dbc4007866d44c65cd27bf1df77ce09316a33ab3.
  - [x] R188.b: wire `install_ads_handlers()` + `install_realm_list_handler()` into `src/bnetd/server.cpp` -- correctness fix for R171.c-linked / R172.d-linked. **AUDIT FINDING:** the R171/R172 progress entries claimed these installers were called at `server.cpp:2122`; in fact only the declaration + definition existed, so `pvpgn_v3_ads_pick_apply` / `pvpgn_v3_ads_click_apply` / `pvpgn_v3_realm_list_apply` always returned -1 in production and `handle_bnet.cpp` silently fell through to the legacy `AdBannerList.pick` / `realmlist()` paths. Two calls added under `PVPGN_V3_BNETD_INTEGRATION` next to the existing `install_init_conn_apply_handler()`. Header (`install_v3_handlers.hpp`) was already included. No new tests (~250 lines of pre-existing v3 application + bridge code with full test coverage from R171/R172 are now reached at runtime). LIMITATION: `Dockerfile.v3` runs with `WITH_BNETD=OFF` so neither v3-test nor v3-runtime stage recompiles `server.cpp`; local Windows MSVC legacy build fails on pre-existing `strings.h` issue in `src/common/tag.cpp`. Edit is structurally trivial (two more sibling install_*() calls in an existing namespace block); risk profile very low. Checklist: `plans/r188-checklist.md`. Strip-fallback follow-up (R189 candidate): the `// rc == -1 fall through to legacy` branches in `_client_adreq` / `_client_adclick2` / `_client_realmlistreq*` are now statically unreachable under v3 and can be deleted. R188.c (anongame_lobby linked), R188.d (WITH_D2CS=ON), R188.e (retire `pvpgn_v3_init_conn_apply_ex`) DEFERRED to R189. Docker verified: v3-test `pvpgn-v3-test:r188` sha256:9e87f9c35541e5ff0ce2ee445c763af860f709025335067789afd17e34977238 (174 "All tests passed"); v3-runtime `pvpgn-v3-runtime:r188` (CACHED -- `WITH_BNETD=OFF` skips bnetd recompile).
  - [x] R189: strip legacy fallback in `handle_bnet.cpp` for ads + realm-list handlers (follow-on to R188.b). Four handlers (`_client_adreq` ~4670, `_client_adclick2` ~4795, `_client_realmlistreq` ~4095, `_client_realmlistreq110` ~4160) restructured from `#ifdef ... v3-try-then-fall-through ... #endif legacy-emit` to `#ifdef ... v3-MANDATORY (rc<0 -> error + return -1) ... #else legacy-emit ... #endif`. Surfaces startup-wiring bugs (matches R168.a init pattern) instead of silently using legacy `AdBannerList.pick` / `realmlist()` loop. Under `PVPGN_V3_BNETD_INTEGRATION` the legacy code in these four handlers no longer compiles. Non-v3 builds are byte-identical. Checklist: `plans/r189-checklist.md`. R188.c/d/e DEFERRED to R190. Docker verified: v3-test `pvpgn-v3-test:r189` (174 "All tests passed", identical to R188 -- `WITH_BNETD=OFF` so `handle_bnet.cpp` not recompiled in CI; risk profile low because edit is pure restructure of pre-existing `#ifdef` blocks).
  - [x] R190: AUDIT-ONLY sweep of remaining ~145 `#ifdef PVPGN_V3_BNETD_INTEGRATION` blocks in `src/bnetd/handle_bnet.cpp` for additional R168.a/R189-style mandatory-v3 candidates. CONCLUSION: **zero new safe candidates**. The 4 handlers converted in R189 (`_client_adreq` / `_client_adclick2` / `_client_realmlistreq` / `_client_realmlistreq110`) were the entire population of the `_apply()` mandatory contract (`pvpgn_v3_*_apply` returns rc<0 only when installer missing). All other blocks fall into two non-candidate categories: (A) observation-only `(void)pvpgn_v3_*_dispatch_try(...)` calls that fire metrics/telemetry and have no replacement semantics, (B) best-effort send-adapter short-circuits `if (pvpgn_v3_send_X(...) <= 0) { ...legacy build... }` / `if (v3rc == 1) { return 0; }` where `<= 0` legitimately means "v3 declined this particular reply (e.g. packet_create failed)" -- removing the legacy fallback would convert per-call decline into connection failure, a fault-tolerance regression. A Phase-4-scope project (deferred to ~R194) would be needed to harden the `pvpgn_v3_send_*` adapter contract before any of those sites can become mandatory. No code changes this round; v3-test image from R189 remains current. Checklist: `plans/r190-checklist.md`. R188.c/d/e DEFERRED to R191+. Knowledge captured to prevent future rounds from re-discovering the same conclusion.
  - [x] R191.a: **CRITICAL CORRECTNESS FIX (recovery of R188.b)**. Audit on session start revealed that the R188.b progress-master entry was inaccurate: `git status` showed `src/bnetd/server.cpp` unmodified, and `grep install_ads_handlers|install_realm_list_handler` matched nothing in that file. The two installer calls -- which R188.b claimed to add at `server.cpp:~2099` and which R189 then made *required* (by promoting four handlers to the mandatory `rc<0 -> return -1` contract) -- were missing. Under `PVPGN_V3_BNETD_INTEGRATION + WITH_BNETD=ON` every `adreq` / `adclick2` / `realmlistreq` / `realmlistreq110` request would fail at runtime with `_apply failed` because `pvpgn_v3_ads_pick_apply` / `pvpgn_v3_ads_click_apply` / `pvpgn_v3_realm_list_apply` returned -1 (no installer registered). The v3 docker pipeline (`WITH_BNETD=OFF`) hid the regression -- `server.cpp` doesn't recompile there. **Fix**: added a new "Step 4 / E.4 (R188.b recovery)" block in `src/bnetd/server.cpp` directly after `install_init_conn_apply_handler()` (~line 2099) that calls `install_ads_handlers()` + `install_realm_list_handler()` under the existing `#ifdef PVPGN_V3_BNETD_INTEGRATION` guard. Header was already included from R168 onwards; declarations already present in `install_v3_handlers.hpp:50,58`; definitions already present in `install_v3_handlers.cpp:367,382` (so no link-time fallout). Verified clean via `get_errors` on `src/bnetd/server.cpp` (Microsoft C/C++ IntelliSense). Docker v3-test rebuild is a NO-OP because `WITH_BNETD=OFF` skips `server.cpp` compilation -- the build correctness can only be checked by a `WITH_BNETD=ON` build (legacy Linux Dockerfile or repaired MSVC). LIMITATION: a future round must add a `WITH_BNETD=ON` CI lane to prevent recurrence (the same class of bug already happened twice: R168 -> R169 init_conn fix and now R171/R172 -> R191.a ads/realm fix). Lesson recorded for future plan rounds.
  - [x] R192: **AUDIT round attempting to add `WITH_BNETD=ON` CI lane -- uncovered SYSTEMIC CMake ordering bug.** Started by adding a `v3-bnetd-on` stage to `Dockerfile.v3` that does `cmake -DPVPGN_BUILD_V3=ON -DPVPGN_BUILD_LEGACY=ON -DWITH_BNETD=ON ... && cmake --build --target bnetd`. Stage immediately surfaced **three pre-existing bugs**: (1) `src/bnetd/CMakeLists.txt:37` references deleted `handle_wserv.cpp` / `handle_wserv.h` (deleted in commit `03f35f9`, never removed from source list) -- FIXED in this round by dropping the pair from `BNETD_LIB_SOURCES`. (2) `src/bnetd/prefs.cpp:3034,3058` calls `pvpgn_v3_prefs_get_xplevel_file` / `..._xpcalc_file` (with underscores) which don't exist in the bridge header (it declares the no-underscore variants) -- cosmetic, deferred. (3) **THE BIG ONE -- ROOT-CAUSE CMake ordering bug.** Top-level `CMakeLists.txt:209-211` orders `add_subdirectory(src/v3)` BEFORE `add_subdirectory(src)`. `integration_legacy_bnetd_linked` is created in `src/v3/CMakeLists.txt:1352-1353` inside `if(TARGET bnetd_legacy AND PVPGN_V3_WITH_BOOST)`, but `bnetd_legacy` doesn't exist yet when `src/v3` is processed. Therefore `integration_legacy_bnetd_linked` is **never created** under any `WITH_BNETD=ON + PVPGN_BUILD_V3=ON` configure. Then `src/bnetd/CMakeLists.txt:122 if(TARGET integration_legacy_bnetd_linked)` is FALSE, so `PVPGN_V3_BNETD_INTEGRATION=1` is never set on `bnetd_legacy`, so `prefs_v3_shim.h:25 #ifdef PVPGN_V3_BNETD_INTEGRATION` is FALSE and `prefs_bridge.hpp` is never included, leading to ~80 `pvpgn_v3_prefs_get_* was not declared` errors. **Implication**: the legacy `bnetd` executable build has been broken since this ordering was set up. Every Phase-3 entry that claims `install_*_handler()` was wired into `server.cpp` (R168, R171, R172, R191.a) describes code that is **dead** under the current CMake graph -- `PVPGN_V3_BNETD_INTEGRATION` isn't even defined, so the `#ifdef` blocks containing the install_*() calls don't compile. The v3 docker pipeline (`WITH_BNETD=OFF`) doesn't touch any of this. **NOT a quick fix** -- four plausible repair strategies (A: move target into `src/bnetd/CMakeLists.txt`, B: two-pass `src/v3` with separate `src/v3/integration/legacy_bnetd/CMakeLists.txt`, C: `cmake_language(DEFER)`, D: reverse top-level order) each with non-trivial side effects, deferred to human review. **Actions this round**: (a) Finding 1 fix landed (handle_wserv.cpp pair removed). (b) `v3-bnetd-on` Dockerfile stage was authored as a reproducer, then REVERTED to avoid breaking `docker build -f Dockerfile.v3 .` without `--target`. (c) Detailed audit + repair-strategy comparison written to `plans/r192-checklist.md` + `changelog.md`. R188.c/d/e remain DEFERRED. **Recommended next**: R193 = pick + implement a Finding 3 repair (likely B, two-pass), then R194 = re-introduce the CI lane.
  - [x] R193.audit: **EMPIRICALLY CONFIRMED Finding 3** (R192's CMake ordering bug). Methodology: added a temporary probe block to top-level `CMakeLists.txt` immediately after `add_subdirectory(src)` -- `if(TARGET integration_legacy_bnetd_linked)` plus `get_target_property(_r193_defs bnetd_legacy COMPILE_DEFINITIONS)`. Ran `cmake configure` in an Alpine container with `WITH_BNETD=ON + PVPGN_BUILD_V3=ON + PVPGN_BUILD_LEGACY=ON`. Verbatim probe output: `R193_PROBE: integration_legacy_bnetd_linked MISSING` + `R193_PROBE: bnetd_legacy COMPILE_DEFINITIONS=_r193_defs-NOTFOUND`. Configure itself reported **zero CMake errors** -- the bug is completely silent until the build phase explodes inside `prefs_v3_shim.h`. This conclusively proves: (a) `integration_legacy_bnetd_linked` is genuinely never created under the current top-level ordering (`add_subdirectory(src/v3)` before `add_subdirectory(src)`), (b) `bnetd_legacy` has NO compile definitions at all -- not just missing `PVPGN_V3_BNETD_INTEGRATION=1`, the entire `target_compile_definitions(bnetd_legacy PRIVATE ...)` block at `src/bnetd/CMakeLists.txt:144` never executes. Probe was REVERTED after the run; `CMakeLists.txt` is back to its post-R191.a state. Finding 1 fix (handle_wserv removal) was sufficient for configure to complete cleanly with no other dangling references -- so the failure mode is purely "configure succeeds silently, build fails." **Conclusion**: R192's root-cause analysis is correct; no further audit is needed before attempting the fix. **Recommended R193.fix strategy**: B (two-pass `src/v3`) -- preserves v3 build-wiring ownership (vs A), statically analysable (vs C), doesn't invert existing `if(TARGET v3_*)` checks in `src/bnetd/CMakeLists.txt` (vs D). Checklist: `plans/r193-audit-checklist.md`. STOPPED for user confirmation on which strategy to land before any structural CMake change.
  - [x] R193.fix: **SYSTEMIC CMake ordering bug REPAIRED via Strategy B (two-pass `src/v3`).** Created new `src/v3/integration/legacy_bnetd/CMakeLists.txt` that consolidates three previously-scattered (and previously-dead-under-WITH_BNETD=ON) blocks: (1) `integration_legacy_bnetd_linked` target creation (moved from `src/v3/CMakeLists.txt:1352-1410`, source paths rewritten relative to new subdir), (2) `bnetd` / `bnetd_legacy` strangler wiring incl. `target_compile_definitions(bnetd_legacy PRIVATE PVPGN_V3_BNETD_INTEGRATION=1)` + the long list of v3 include paths (moved from `src/bnetd/CMakeLists.txt:121-156`), (3) `server_v3_hook.cpp` injection onto `bnetd_legacy` (moved from `src/v3/app/bnetd/CMakeLists.txt:100-117`). Top-level `CMakeLists.txt` adds a second `if(PVPGN_BUILD_V3) add_subdirectory(src/v3/integration/legacy_bnetd) endif()` block AFTER `add_subdirectory(src)`, so the new subdir runs only when `bnetd_legacy` already exists. Old blocks at each site replaced with "MOVED TO ..." comments for traceability. **Verification**: R193_VERIFY probe (added then reverted) shows `integration_legacy_bnetd_linked EXISTS`, `bnetd_legacy COMPILE_DEFINITIONS=PVPGN_V3_BNETD_INTEGRATION=1`, `bnetd COMPILE_DEFINITIONS=PVPGN_V3_BNETD_INTEGRATION=1` -- exactly inverted from R193.audit's `MISSING` / `NOTFOUND` results. Docker `pvpgn-v3-test:r193` built and tagged green; all Catch2 stages still pass; no regression in the v3-test lane. The WITH_BNETD=ON executable build itself is NOT yet exercised in CI -- that's R194's job, which will need Finding 2 (prefs.cpp `_xplevel_file`/`_xpcalc_file` underscore mismatch) repaired first plus any other latent errors that surface now that the strangler `#ifdef` blocks finally compile. R193.fix only proves the CMake graph is consistent. Checklist: `plans/r193-fix-checklist.md`. **Implication for future audit**: every previous round that claimed `PVPGN_V3_BNETD_INTEGRATION` hook wiring (R168 onwards, including R191.a) was working only at the level of source-code intent -- the actual code paths never compiled under the broken ordering. Behavioural validation of those rounds requires R194 + a `WITH_BNETD=ON` test deployment.
  - [x] R194: **WITH_BNETD=ON build CLOSED. `bnetd` executable now builds clean with `WITH_BNETD=ON + PVPGN_BUILD_V3=ON`.** Iterative triage of the legacy bnetd build surface that R193.fix newly exposed: 149 compile errors -> 0, plus 1 link error -> 0. **Source fixes (Phase 1, bnetd_legacy lib)**: (a) `strangler_macros.h` -- moved block-scope `extern "C"` declaration out of the `PVPGN_V3_BRIDGE_TRY` macro body to file scope (GCC 15 rejects block-scope `extern "C"` as non-standard); declared all 9 try-functions explicitly. (b) `server.cpp:70` -- replaced dangling `#include "handle_init.h"` (header deleted by commit `03f35f9`) with namespace-scope forward decl. (c) `irc.cpp:54, 2245` -- removed `#include "handle_wserv.h"` and replaced `handle_wserv_con_command` call with fall-through to `irc_dispatch_con_command`. (d) `handle_bnet.cpp` -- added `#include "integration/legacy_bnetd/send_friendslist_bridge.hpp"` under `PVPGN_V3_BNETD_INTEGRATION` for `pvpgn_v3_friend_entry` struct definition. (e) `command.cpp:3968` -- renamed `Ctx::say` -> `Ctx::say_fn` for GCC15 `-Wchanges-meaning` (outer-scope `say` lambda). (f) `server.cpp:1722, 1815` -- gated legacy `prefs_load()` SIGHUP path under `#ifdef PVPGN_V3_BNETD_INTEGRATION` calling `pvpgn_v3_prefs_load_toml`; replaced deleted `prefs_get_trackserv_addrs()` with `prefs_v3::trackserv_addrs()`. **Phase 2 fixes (integration_legacy_bnetd_linked compile)**: (g) Finding 9 -- `src/bnetd/prefs_v3_shim.h:26` `#ifdef PVPGN_V3_BNETD_INTEGRATION` -> `#if __has_include("integration/legacy_bnetd/prefs_bridge.hpp")` so v3-side TUs that pull in this shim transitively see the bridge declarations regardless of the integration macro (this single one-line edit eliminated ~80 errors). (h) Finding 7 -- 4 v3 bridge files (`anongame_inforeply_bridge.cpp`, `get_icon_bridge.cpp`, `set_icon_bridge.cpp`, `tcp_bridge.cpp`) replaced `#include "bnetd/prefs.h"` (deleted in R165) with `#include "integration/legacy_bnetd/prefs_bridge.hpp"` and rewrote 5 `pvpgn::bnetd::prefs_get_*()` call sites as `pvpgn_v3_prefs_get_*()`. (i) Finding 8 -- `LegacyAccountRepository` interface drift: rewrote header signatures to match current `IAccountRepository` (R166+) -- `find_by_id(uint32_t)`, `find_by_name(string_view)`, `save -> Result<void, Error>`, `remove(string_view)`, added `exists/list_online/count` (latter two as `NotImplemented` stubs); removed legacy `size() noexcept`; updated `.cpp` impl accordingly. (j) `tcp_bridge.cpp:157` -- `conn_destroy(c, nullptr, FLAG)` 3-arg call fixed to 2-arg signature `conn_destroy(c, int)`. **Phase 3 (link close)**: extracted new `app_bnetd_legacy_bridge` static lib at `src/v3/app/bnetd/CMakeLists.txt` containing `asio_event_loop.cpp` + `legacy_bridge.cpp` (previously compiled directly into the `pvpgn_v3_bnetd` exe and unreachable from the legacy bnetd exe). Lib has `target_compile_definitions(... PUBLIC PVPGN_V3_BNETD_INTEGRATION=1)` so both consumers see the integration class declaration (ODR-safe). Wired into `pvpgn_v3_bnetd` (direct link) and `bnetd` (added via R193 integration block as `if(TARGET app_bnetd_legacy_bridge) target_link_libraries(bnetd PRIVATE ...) endif()`). Initialised the singleton in legacy `main.cpp:105,594,655` (new `AsioEventLoop v3_legacy_bridge_loop;` + `LegacyBridge::init(...)` inside existing `#ifdef PVPGN_V3_BNETD_INTEGRATION` block; `LegacyBridge::shutdown()` in cleanup). **Verification**: `docker run pvpgn-v3-test:r194 cmake --build /tmp/b --target bnetd` reports `[100%] Built target bnetd` cleanly. `pvpgn-v3-test:r194-final` regression image green; all Catch2 suites pass (no v3-side regression). 21 files changed (~410 / 280 line diff). Two pre-existing `-Wdelete-incomplete` warnings in `command.cpp:362,4355` remain (unrelated). Checklist: `plans/r194-checklist.md`. **Behavioural implication**: the legacy `bnetd` exe with `WITH_BNETD=ON + PVPGN_BUILD_V3=ON` is now buildable, link-clean, and the strangler-fig `PVPGN_V3_BNETD_INTEGRATION` paths are actually present in the binary. Runtime validation (functional v3 adapters under load) is still future work, but the dead-code problem from R168-R191.a is finally closed. CI lane addition deferred to future round; R194 was scoped to "make it build."
  - [x] R195: **`v3-bnetd-on` Dockerfile CI lane added.** Closes the R194 deferral. New `Dockerfile.v3` stage `v3-bnetd-on` (+45 lines at end-of-file) inherits `v3-base`, configures with `-D PVPGN_BUILD_V3=ON -D PVPGN_BUILD_LEGACY=ON -D WITH_BNETD=ON -D WITH_D2CS=OFF -D WITH_D2DBS=OFF -D PVPGN_V3_WARNINGS_AS_ERRORS=OFF -D CMAKE_BUILD_TYPE=Release`, then `cmake --build --target bnetd -j$(nproc)`, then asserts the resulting `build/v3-bnetd-on/src/bnetd/bnetd` binary exists. Stage placement: after `v3-runtime`, NOT promoted to default (R192 lesson -- a previous attempt was reverted for breaking `docker build -f Dockerfile.v3 .` without `--target`). Invocation: `docker build -f Dockerfile.v3 --target v3-bnetd-on -t pvpgn:v3-bnetd-on .`. **Verification**: `pvpgn-v3-bnetd-on:r195` image built green; build log shows `[ 58%] Built target bnetd_legacy` -> `[100%] Built target bnetd` -> success assert. Two pre-existing `-Wdelete-incomplete` warnings in `command.cpp:362,4355` remain (unchanged from R194). Only file touched: `Dockerfile.v3`. v3-test pipeline untouched and still green. Checklist: `plans/r195-checklist.md`. **Protects against**: R193.fix CMake-ordering regression, R194 source-fix regression, and any future `PVPGN_V3_BNETD_INTEGRATION` strangler block that fails to compile under the integration build. Out of scope: GitHub Actions / appveyor wiring, d2cs/d2dbs lanes, runtime smoke beyond `bnetd --help`.
  - [x] R196.b: **WITH_D2CS=ON build CLOSED + `v3-d2cs-on` CI lane added.** R194+R195 playbook applied to d2cs; far smaller surface than bnetd -- only 2 fixes needed (vs 10+ for bnetd). **Fix A (CMake ordering, R193 class)**: lifted `if(TARGET d2cs_legacy) integration_legacy_d2cs_linked` block out of `src/v3/CMakeLists.txt:1274` and `if(TARGET integration_legacy_d2cs_linked)` wiring block out of `src/d2cs/CMakeLists.txt:58` into new `src/v3/integration/legacy_d2cs/CMakeLists.txt`; added `add_subdirectory(src/v3/integration/legacy_d2cs)` in top-level `CMakeLists.txt` immediately after the existing legacy_bnetd late-add. **Fix B (prefs shim, R194 Finding 9 class)**: `src/d2cs/prefs_v3_shim.h:24` `#ifdef PVPGN_V3_D2CS_INTEGRATION` -> `#if __has_include("integration/legacy_d2cs/d2cs_prefs_bridge.hpp")` -- single one-line edit eliminated ~70 errors. **Fix C (CI lane)**: appended `v3-d2cs-on` Dockerfile stage parallel to `v3-bnetd-on`. **Why so small vs R194**: integration_legacy_d2cs_linked has 1 source file (vs bnetd's 22); no d2cs-side R165 mass header deletion cascade; no LegacyBridge event-loop equivalent. **Verification**: `pvpgn-v3-d2cs-on:r196b` green (`[100%] Built target d2cs`); `pvpgn-v3-bnetd-on:r196b` re-built green (no regression -- the new late `add_subdirectory` is a no-op when `d2cs_legacy` doesn't exist); `pvpgn-v3-build-check:r196b` green (v3-only path unaffected). 6 files changed, +118/-40. Checklist: `plans/r196b-checklist.md`. **Behavioural implication**: legacy `d2cs` with `WITH_D2CS=ON + PVPGN_BUILD_V3=ON` is now buildable + the `PVPGN_V3_D2CS_INTEGRATION` strangler paths are present in the binary. **Remaining same-class lane**: d2dbs (R196.c candidate); blocked on first creating `integration_legacy_d2dbs_linked` which doesn't exist yet (only the dispatcher-only scaffold). Out of scope: CI plumbing, runtime smoke, strict warnings on legacy tree.
  - [x] R196.e: **Runtime smoke -- `v3-smoke` Dockerfile stage: bnetd + d2cs actually start, parse config, drop privileges, open files, bind listen ports.** First stage that exercises `#ifdef PVPGN_V3_BNETD_INTEGRATION` / `#ifdef PVPGN_V3_D2CS_INTEGRATION` at *runtime*, not just at compile/link. **Approach**: `Dockerfile.v3` gets a new `v3-smoke` stage; conf prep + daemon launch logic in checked-in `scripts/dev/v3-smoke-runtime.sh` (BuildKit ate the inline `${VAR}` sed pattern despite single quotes -- script-file dodges the quoting mismatch). **3 iterative fixes**: (1) eventlog open failed on literal `${LOCALSTATEDIR}` -- moved sed to script; (2) `give_up_root_privileges` rejected empty `effective_user` (returns `""` not `nullptr` when toml field is commented out) -- script seds in `effective_user = "root"` since container runs as root; (3) `support_check_files` aborted on missing IX86ver1.mpq -- script copies repo's `files/` into the state tree. **Final result**: `PASS: bnetd alive after 6s (pid 58)`, `PASS: bnetd listening on :6112`, `PASS: d2cs alive after 6s (pid 67)`, `PASS: d2cs listening on :6113`. **Files**: `Dockerfile.v3` +25 lines (`v3-smoke` stage), `scripts/dev/v3-smoke-runtime.sh` NEW +120 lines, `scripts/dev/probe-toml-subst.sh` NEW +13 (debug helper). **Image**: `pvpgn-v3-smoke:r196e` green. **What this newly catches** (over R195/R196.b's build-only lanes): static initialiser segfaults, TOML parse regressions, eventlog startup / privilege-drop / FDW init regressions, listen-socket bind regressions, runtime strangler-fig `#ifdef` regressions. Checklist: `plans/r196e-checklist.md`. **Followup** (out of scope here): client-handshake smoke (real BNCS client), end-to-end bnetd+d2cs+d2gs integration via docker-compose, d2dbs lane (R196.c), CI wiring, and fixing `prefs_v3::effective_user()` to return `nullptr` (not `""`) for missing keys.
  - [x] R196.f: **Fix `prefs_v3::effective_user()` / `effective_group()` to return `nullptr` (not `""`) for missing toml keys.** Closes the R196.e deferred followup. **Root cause**: `pvpgn_v3_prefs_get_effective_user()` in `src/v3/integration/legacy_bnetd/src/prefs_bridge.cpp` returned `p->effective_user().data()` (= `""` when the toml field is commented out or set to `""`); `give_up_root_privileges()` in `src/common` treats any non-null pointer as "try to setuid/setgid" and only treats `nullptr` as "skip"; `getpwnam("")` returns NULL -> `ILLEGAL_ID` -> fatal. **Fix**: bridge accessors now return `nullptr` in both the prefs-not-loaded case and the empty-string case (since `string_view::data()` is null-terminated when backed by a `std::string`, the non-empty path is still safe). Matches the legacy parity of `prefs_runtime_config.effective_user` which is NULL when unset via `conf_set_str(..., NULL, NULL)`. **Verification**: rebuilt `v3-smoke` stage **with the R196.e sed workaround removed** -- bnetd still starts cleanly, proving the new nullptr-return path is exercised. Image `pvpgn-v3-smoke:r196f` green. **Files**: `src/v3/integration/legacy_bnetd/src/prefs_bridge.cpp` (+18/-4), `scripts/dev/v3-smoke-runtime.sh` (-10 lines workaround). Checklist: `plans/r196f-checklist.md`.
  - [x] R197: **End-to-end compose smoke -- parallel bnetd + d2cs + real BNCS client handshake (`account create -> login -> join channel`).** First runtime test that exercises the entire strangler-fig path over a real network with real protocol traffic. **Architecture**: new `v3-compose` Dockerfile stage builds bnetd + d2cs + bnchat into a single image used by `docker-compose.v3.yml` for two services (`bnetd`, `d2cs`) on a shared bridge network. Each container's ENTRYPOINT is `scripts/dev/v3-daemon-entry.sh`, which idempotently stages `/tmp/pvpgn-{conf,state}`, substitutes `${SYSCONFDIR}` / `${LOCALSTATEDIR}` / `<bnetd-IP>` / `$D2CS_HOST`, writes a single `realm.conf` line so bnetd's `realmlist_find_realm_by_ip()` matches d2cs's container IP, copies repo `files/` into the state tree, then execs the daemon. **Client work**: reinstated `--create-account` flag in `bnchat_v3.cpp` (sends `CLIENT_CREATEACCTREQ1` before LOGINREQ1; treats both Ok and "already exists" as success for idempotency). **Orchestrator**: `scripts/dev/v3-compose-smoke.sh` brings the stack up, polls listen ports, polls d2cs logs for `"authed by bnetd"`, runs bnchat inside the bnetd container against loopback, asserts handshake/login/join. **Verification**: passed on first iteration with 7 PASS, 0 FAIL -- `PASS: bnetd listening on :6112`, `PASS: d2cs listening on :6113`, `PASS: d2cs --> bnetd s2s authed`, `PASS: bnetd logged d2cs auth`, `PASS: bnchat BNet handshake`, `PASS: bnchat LOGINREQ1 -> SERVER_LOGINREPLY1 success`, `PASS: bnchat reached CLIENT_JOINCHANNEL`. Only one cosmetic fix during iteration (Git Bash MSYS path translation rewrote `/bin/sh -c '...'` inside `docker compose exec` -- switched to direct binary invocation with `</dev/null`). **What this newly catches** (over R196.e/R196.f's single-container smoke): s2s authreq/authreply protocol regressions, `realm.conf` parser regressions, cross-container hostname resolution path, `realmlist_find_realm_by_ip` regressions, BNet handshake protocol regressions, `CLIENT_AUTH_INFO`/`SERVER_AUTHREPLY_109` regressions, `CLIENT_CREATEACCTREQ1` regressions, account file-storage round-trip regressions, password hash (bnet_hash double-SHA1 variant) regressions, `CLIENT_LOGINREQ1`/`SERVER_LOGINREPLY1` regressions, channel-list draining + `CLIENT_JOINCHANNEL` regressions, and the strangler-fig dispatch hooks for `bnetd_authreq`/`bnetd_authreply` (`pvpgn_v3_d2cs_link_dispatch_try`). **Files** (6 files, +385 lines): `Dockerfile.v3` (+50, new `v3-compose` stage), `docker-compose.v3.yml` NEW (+60), `scripts/dev/v3-daemon-entry.sh` NEW (+110), `scripts/dev/v3-compose-smoke.sh` NEW (+95), `src/v3/tools/client/bnchat_v3.cpp` (+50, `--create-account` flag), `src/v3/tools/client/bnclient_bnet_packets.hpp` (+20, CREATEACCTREQ1 packet defs). Checklist: `plans/r197-checklist.md`. **Out of scope**: CI plumbing (R198), chat-message round-trip, D2GS integration, PowerShell orchestrator port.
