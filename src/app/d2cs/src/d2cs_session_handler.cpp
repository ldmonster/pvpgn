    ...
    const protocol::d2cs::D2CSCreateCharRequest& req)
{
    ...
    // A freshly created character carries the INIT status bit, mirroring the
    // original's d2char_create (charstatus_set_init): the portrait status byte
    // the client reads back in CHARLISTREPLY therefore includes 0x01.
    info.flags   = static_cast<domain::d2cs::CharacterFlags>(
                       static_cast<uint8_t>(req.char_status))
                   | domain::d2cs::CharacterFlags::Init;
    info.level   = 1;
    info.experience = 0;
    info.last_played = 0;
