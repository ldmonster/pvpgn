    ...
    switch (client_tag) {
        ...
        default:
            return core::fail(core::make_error(
                ...
                "anongame_infoply: unknown client tag"));
    }
}
...
core::Result<pb::AnonGameInfoReply> build_inforeply_for_tag(
    ...
    if (!ser.value().found) {
        return core::fail(core::make_error(
            ...
            "anongame_infoply: snapshot has no payload for requested tag"));
    }
...
core::Result<pb::AnonGameInfoReply> build_inforeply_for_tag(
    ...
    if (framed == nullptr) {
        return core::fail(core::make_error(
            core::StatusCode::NotFound,
            "anongame_infoply: compiled snapshot has no payload for tag"));
    }
    auto stag = server_tag_for(client_tag);
    if (!stag) return core::fail(stag.error());
