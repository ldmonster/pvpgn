...
namespace pvpgn
{
	...
	extern int list_destroy(t_list * list)
	{
		if (!list)
		{
			LOG_ERROR(__FUNCTION__, "got NULL list");
			return -1;
		}

		if (list->head)
			LOG_ERROR(__FUNCTION__, "got non-empty list");
	...
	extern int list_remove_elem(t_list * list, t_elem ** elem)
	{
		...
		if (!elem)
		{
			LOG_ERROR(__FUNCTION__, "got NULL *elem");
			return -1;
		}
	...
	extern void * list_get_data_by_pos(t_list const * list, unsigned int pos)
	{
		...
		LOG_ERROR(__FUNCTION__, "requested position {} but len={}", pos, len);
		return NULL;
	}
...
#endif
	{
		...
		{
#ifdef LIST_DEBUG
			LOG_ERROR(__FUNCTION__, "got NULL list from {}:{}", fn, ln);
#else
			LOG_ERROR(__FUNCTION__, "got NULL list");
#endif
			return NULL;
	...
	extern t_elem * elem_get_next_real(t_list const * list, t_elem const * elem, char const * fn, unsigned int ln)
	{
		if (!elem)
		{
			LOG_ERROR(__FUNCTION__, "got NULL elem from {}:{}", fn, ln);
			return NULL;
		}
