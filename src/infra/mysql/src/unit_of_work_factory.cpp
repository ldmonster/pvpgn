...
MySQLUnitOfWorkFactory::MySQLUnitOfWorkFactory(std::string_view connection_string)
...
void MySQLUnitOfWorkFactory::parse_connection_string() {
...
MySQLUnitOfWorkFactory::create() {
    auto conn = std::make_shared<MySQLConnection>(
        host_, port_, user_, password_, database_);
    return std::make_unique<MySQLUnitOfWork>(std::move(conn));
}
