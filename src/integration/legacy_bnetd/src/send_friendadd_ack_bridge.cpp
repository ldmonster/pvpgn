// SPDX-License-Identifier: GPL-2.0-or-later
#include "integration/legacy_bnetd/send_friendadd_ack_bridge.hpp"

#include <cstdint>

#include "integration/legacy_bnetd/send_packet_bridge.hpp"
#include "protocol/bnet/codec.hpp"
#include "protocol/bnet/messages.hpp"
#include "protocol/common/writer.hpp"

extern "C" int pvpgn_v3_send_friendadd_ack(void* conn_ptr,
                                            char const* name,
                                            unsigned int status,
                                            unsigned int location,
                                            unsigned int client_tag,
                                            char const* location_name) noexcept {
    if (conn_ptr == nullptr) return 0;
    if (name == nullptr) return 0;
    if (location_name == nullptr) return 0;

    pvpgn::protocol::bnet::FriendAddAck m;
    m.name          = name;
    m.status        = static_cast<std::uint8_t>(status & 0xffu);
    m.location      = static_cast<std::uint8_t>(location & 0xffu);
    m.client_tag    = static_cast<std::uint32_t>(client_tag);
    m.location_name = location_name;

    pvpgn::protocol::Writer w;
    auto enc = pvpgn::protocol::bnet::encode(w, m);
    if (!enc.has_value()) return 0;

    auto bytes = w.take();
    if (bytes.empty()) return 0;
    if (bytes.size() > pvpgn::integration::legacy_bnetd::kSendPacketMaxSize) {
        return 0;
    }
    return ::pvpgn_v3_send_packet(
        conn_ptr,
        bytes.data(),
        static_cast<unsigned int>(bytes.size()));
}
