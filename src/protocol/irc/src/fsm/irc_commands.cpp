...
/// @file irc_commands.cpp
/// IrcFsm — post-registration command handlers.
///
/// Handles all commands available after the Greeting phase:
///
///   Always-available (any non-Closing state):
///     on_ping()    — PING → PONG
///     on_quit()    — QUIT → close session
///     on_motd()    — MOTD → 375 + 376
///
///   Post-registration (Registered or InChannel):
///     on_join()    — JOIN #channel
///     on_part()    — PART #channel
///     on_privmsg() — PRIVMSG <target> :<text>
///     on_notice()  — NOTICE <target> :<text>
///     on_away()    — AWAY [message]
///     on_whois()   — WHOIS <nick>
///     on_who()     — WHO <mask>
///     on_mode()    — MODE <target> [modes]
///     on_topic()   — TOPIC #channel [new_topic]
///     on_names()   — NAMES [#channel]
///     on_kick()    — KICK #channel <target> [reason]
///     on_list()    — LIST [#channel_filter]
