...
void parse_storage(const Config& cfg, ServerConfig& sc)
{
    if (auto sec = cfg.section("storage")) {
        ...
        sc.storage.dsn    = sec->get_or<std::string>("dsn",    sc.storage.dsn);
...
void parse_wol(const Config& cfg, ServerConfig& sc)
{
    if (auto sec = cfg.section("wol")) {
        ...
        sc.wol.wol_autoupdate_password    = sec->get_or<std::string>("wol_autoupdate_password",    sc.wol.wol_autoupdate_password);
    }
}
