...
def run(oracle, v3):
    ...
    for idx, spec in enumerate(cat.representatives()):
        ...
        # --- v3 (all families now supported, incl. WOL CVERS/APGAR auth) ---
        try:
            n = cat.login(spec, "127.0.0.1", v3.port, user, pw,
                          wol_port=(v3.wol_port if spec.family == "wol" else None))
...
def main():
    ...
    print(f"\noracle logins OK: {oracle_ok}/{len(rows)}   "
          f"match v3: {matched}/{len(rows)}")
    if ok_all:
        print("\nAll supported-client mocks log in against the oracle and "
              "match v3 (OLS + NLS + WOL).")
        return 0
    print("\nFAIL: an oracle login failed or a path diverged from v3.")
    return 1
