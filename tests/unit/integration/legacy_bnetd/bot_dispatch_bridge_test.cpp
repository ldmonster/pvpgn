...
#include "integration/legacy_bnetd/bot_dispatch_bridge.hpp"
...
TEST_CASE("bot dispatch bridge null conn no-op",
          "[integration][legacy_bnetd][bot_dispatch_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_bot_dispatch_try(nullptr, "connected") == 0);
    REQUIRE(sink.records.empty());
}

TEST_CASE("bot dispatch bridge logs connected state",
          "[integration][legacy_bnetd][bot_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_bot_dispatch_try(&marker, "connected") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(sink.records[0].module == "v3_bot_dispatch_bridge");
    REQUIRE(sink.records[0].message == "bot dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "connected");
}

TEST_CASE("bot dispatch bridge logs bot_username state",
          "[integration][legacy_bnetd][bot_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_bot_dispatch_try(&marker, "bot_username") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "bot_username");
}

TEST_CASE("bot dispatch bridge logs bot_password state",
          "[integration][legacy_bnetd][bot_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_bot_dispatch_try(&marker, "bot_password") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "bot_password");
}

TEST_CASE("bot dispatch bridge logs loggedin state",
          "[integration][legacy_bnetd][bot_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_bot_dispatch_try(&marker, "loggedin") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "loggedin");
}

TEST_CASE("bot dispatch bridge null op normalised",
          "[integration][legacy_bnetd][bot_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_bot_dispatch_try(&marker, nullptr) == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "?");
}

TEST_CASE("bot dispatch bridge empty op normalised",
          "[integration][legacy_bnetd][bot_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_bot_dispatch_try(&marker, "") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "?");
}
