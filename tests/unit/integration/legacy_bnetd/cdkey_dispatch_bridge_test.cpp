...
#include "integration/legacy_bnetd/cdkey_dispatch_bridge.hpp"
...
TEST_CASE("cdkey dispatch bridge null conn no-op",
...
TEST_CASE("cdkey dispatch bridge logs cdkey",
          "[integration][legacy_bnetd][cdkey_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    REQUIRE(field_value(sink.records[0], "op") == "cdkey");
}

TEST_CASE("cdkey dispatch bridge logs cdkey2",
          "[integration][legacy_bnetd][cdkey_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("cdkey dispatch bridge logs cdkey3",
          "[integration][legacy_bnetd][cdkey_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("cdkey dispatch bridge null op normalised",
          "[integration][legacy_bnetd][cdkey_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
