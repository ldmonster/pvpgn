...
// Unit tests for the coalesced clan packet-send observation bridge.
...
TEST_CASE("clan send bridge null conn no-op",
...
TEST_CASE("clan send bridge logs op",
          "[integration][legacy_bnetd][clan_send_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    REQUIRE(r.module  == "v3_clan_send_bridge");
    REQUIRE(r.message == "clan send observed");
    REQUIRE(field_value(r, "op") == "clan_send_memberlist");
}

TEST_CASE("clan send bridge null op normalised",
          "[integration][legacy_bnetd][clan_send_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
