...
TEST_CASE("command dispatch bridge null conn no-op", "[integration][legacy_bnetd][command_dispatch_bridge]") {
    RecordingLogger sink; ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_command_dispatch_try(nullptr, "/help") == 0);
    REQUIRE(sink.records.empty());
}

TEST_CASE("command dispatch bridge logs command text", "[integration][legacy_bnetd][command_dispatch_bridge]") {
    int marker = 0; RecordingLogger sink; ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_command_dispatch_try(&marker, "/help") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(sink.records[0].module == "v3_command_dispatch_bridge");
    REQUIRE(sink.records[0].message == "command dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "/help");
}

TEST_CASE("command dispatch bridge null op normalised", "[integration][legacy_bnetd][command_dispatch_bridge]") {
    int marker = 0; RecordingLogger sink; ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_command_dispatch_try(&marker, nullptr) == 0);
    REQUIRE(field_value(sink.records[0], "op") == "?");
}

TEST_CASE("command dispatch bridge empty op normalised", "[integration][legacy_bnetd][command_dispatch_bridge]") {
    int marker = 0; RecordingLogger sink; ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_command_dispatch_try(&marker, "") == 0);
    REQUIRE(field_value(sink.records[0], "op") == "?");
}
