...
#include "integration/legacy_bnetd/d2_character_dispatch_bridge.hpp"
...
TEST_CASE("d2 character dispatch bridge null conn no-op",
          "[integration][legacy_bnetd][d2_character_dispatch_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_d2_character_dispatch_try(nullptr,
                                                  "charlistreq") == 0);
    REQUIRE(sink.records.empty());
}

TEST_CASE("d2 character dispatch bridge logs charlistreq",
          "[integration][legacy_bnetd][d2_character_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_d2_character_dispatch_try(&marker,
                                                  "charlistreq") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(sink.records[0].module
            == "v3_d2_character_dispatch_bridge");
    REQUIRE(sink.records[0].message
            == "d2 character dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "charlistreq");
}

TEST_CASE("d2 character dispatch bridge null op normalised",
          "[integration][legacy_bnetd][d2_character_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("d2 character dispatch bridge empty op normalised",
          "[integration][legacy_bnetd][d2_character_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_d2_character_dispatch_try(&marker, "") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "?");
}
