...
#include "integration/legacy_bnetd/file_dispatch_bridge.hpp"
...
TEST_CASE("file dispatch bridge null conn no-op",
...
TEST_CASE("file dispatch bridge logs file_req",
          "[integration][legacy_bnetd][file_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    REQUIRE(field_value(sink.records[0], "op") == "file_req");
}

TEST_CASE("file dispatch bridge logs file_req2",
          "[integration][legacy_bnetd][file_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("file dispatch bridge logs file_req3",
          "[integration][legacy_bnetd][file_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("file dispatch bridge null op normalised",
          "[integration][legacy_bnetd][file_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
