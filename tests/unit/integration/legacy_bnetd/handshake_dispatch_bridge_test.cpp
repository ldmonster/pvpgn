...
#include "integration/legacy_bnetd/handshake_dispatch_bridge.hpp"
...
TEST_CASE("handshake dispatch bridge null conn no-op",
          "[integration][legacy_bnetd][handshake_dispatch_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_handshake_dispatch_try(nullptr,
                                               "motdw3") == 0);
    REQUIRE(sink.records.empty());
}

TEST_CASE("handshake dispatch bridge logs motdw3",
          "[integration][legacy_bnetd][handshake_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_handshake_dispatch_try(&marker,
                                               "motdw3") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(sink.records[0].module
            == "v3_handshake_dispatch_bridge");
    REQUIRE(sink.records[0].message
            == "handshake dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "motdw3");
}

TEST_CASE("handshake dispatch bridge logs compinfo1",
          "[integration][legacy_bnetd][handshake_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_handshake_dispatch_try(&marker,
                                               "compinfo1") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "compinfo1");
}

TEST_CASE("handshake dispatch bridge logs compinfo2",
          "[integration][legacy_bnetd][handshake_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_handshake_dispatch_try(&marker,
                                               "compinfo2") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "compinfo2");
}

TEST_CASE("handshake dispatch bridge logs countryinfo1",
          "[integration][legacy_bnetd][handshake_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_handshake_dispatch_try(&marker,
                                               "countryinfo1") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "countryinfo1");
}

TEST_CASE("handshake dispatch bridge null op normalised",
          "[integration][legacy_bnetd][handshake_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_handshake_dispatch_try(&marker, nullptr) == 0);
    REQUIRE(sink.records.size() == 1u);
