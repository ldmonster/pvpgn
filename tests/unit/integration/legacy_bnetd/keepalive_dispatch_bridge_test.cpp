...
#include "integration/legacy_bnetd/keepalive_dispatch_bridge.hpp"
...
TEST_CASE("keepalive dispatch bridge null conn no-op",
...
TEST_CASE("keepalive dispatch bridge logs pingreq",
          "[integration][legacy_bnetd][keepalive_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    REQUIRE(sink.records[0].module ==
            "v3_keepalive_dispatch_bridge");
    REQUIRE(sink.records[0].message ==
            "keepalive dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "pingreq");
}

TEST_CASE("keepalive dispatch bridge logs echoreply",
          "[integration][legacy_bnetd][keepalive_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("keepalive dispatch bridge null op normalised",
          "[integration][legacy_bnetd][keepalive_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
