...
#include "integration/legacy_bnetd/ladder_dispatch_bridge.hpp"
...
TEST_CASE("ladder dispatch bridge null conn no-op",
...
TEST_CASE("ladder dispatch bridge logs ladderreq",
          "[integration][legacy_bnetd][ladder_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    REQUIRE(sink.records[0].message == "ladder dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "ladderreq");
}

TEST_CASE("ladder dispatch bridge logs laddersearchreq",
          "[integration][legacy_bnetd][ladder_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("ladder dispatch bridge null op normalised",
          "[integration][legacy_bnetd][ladder_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
