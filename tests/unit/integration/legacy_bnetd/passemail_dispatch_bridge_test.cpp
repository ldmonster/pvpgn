...
#include "integration/legacy_bnetd/passemail_dispatch_bridge.hpp"
...
TEST_CASE("passemail dispatch bridge null conn no-op",
          "[integration][legacy_bnetd][passemail_dispatch_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_passemail_dispatch_try(nullptr,
                                               "passchangereq") == 0);
    REQUIRE(sink.records.empty());
}

TEST_CASE("passemail dispatch bridge logs passchangereq",
          "[integration][legacy_bnetd][passemail_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_passemail_dispatch_try(&marker,
                                               "passchangereq") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(sink.records[0].module
            == "v3_passemail_dispatch_bridge");
    REQUIRE(sink.records[0].message
            == "passemail dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "passchangereq");
}

TEST_CASE("passemail dispatch bridge logs passchangeproofreq",
          "[integration][legacy_bnetd][passemail_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_passemail_dispatch_try(&marker,
                                               "passchangeproofreq") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op")
            == "passchangeproofreq");
}

TEST_CASE("passemail dispatch bridge logs getpasswordreq",
          "[integration][legacy_bnetd][passemail_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_passemail_dispatch_try(&marker,
                                               "getpasswordreq") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "getpasswordreq");
}

TEST_CASE("passemail dispatch bridge null op normalised",
          "[integration][legacy_bnetd][passemail_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_passemail_dispatch_try(&marker, nullptr) == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "?");
}
