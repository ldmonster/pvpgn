...
#include "integration/legacy_bnetd/profile_dispatch_bridge.hpp"
...
TEST_CASE("profile dispatch bridge null conn no-op",
          "[integration][legacy_bnetd][profile_dispatch_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_profile_dispatch_try(nullptr,
                                             "profilereq") == 0);
    REQUIRE(sink.records.empty());
}

TEST_CASE("profile dispatch bridge logs profilereq",
          "[integration][legacy_bnetd][profile_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_profile_dispatch_try(&marker,
                                             "profilereq") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(sink.records[0].module == "v3_profile_dispatch_bridge");
    REQUIRE(sink.records[0].message == "profile dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "profilereq");
}

TEST_CASE("profile dispatch bridge logs statsreq",
          "[integration][legacy_bnetd][profile_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_profile_dispatch_try(&marker,
                                             "statsreq") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "statsreq");
}

TEST_CASE("profile dispatch bridge logs statsupdate",
          "[integration][legacy_bnetd][profile_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_profile_dispatch_try(&marker,
                                             "statsupdate") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "statsupdate");
}

TEST_CASE("profile dispatch bridge null op normalised",
          "[integration][legacy_bnetd][profile_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_profile_dispatch_try(&marker, nullptr) == 0);
    REQUIRE(sink.records.size() == 1u);
