...
#include "integration/legacy_bnetd/progident_dispatch_bridge.hpp"
...
TEST_CASE("progident dispatch bridge null conn no-op",
...
TEST_CASE("progident dispatch bridge logs progident",
          "[integration][legacy_bnetd][progident_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    REQUIRE(sink.records[0].module
            == "v3_progident_dispatch_bridge");
    REQUIRE(sink.records[0].message
            == "progident dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "progident");
}

TEST_CASE("progident dispatch bridge logs progident2",
          "[integration][legacy_bnetd][progident_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("progident dispatch bridge logs changeclient",
          "[integration][legacy_bnetd][progident_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("progident dispatch bridge null op normalised",
          "[integration][legacy_bnetd][progident_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
