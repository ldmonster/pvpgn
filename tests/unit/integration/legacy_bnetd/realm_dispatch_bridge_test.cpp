...
#include "integration/legacy_bnetd/realm_dispatch_bridge.hpp"
...
TEST_CASE("realm dispatch bridge null conn no-op",
          "[integration][legacy_bnetd][realm_dispatch_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_realm_dispatch_try(nullptr,
                                           "realmlistreq") == 0);
    REQUIRE(sink.records.empty());
}

TEST_CASE("realm dispatch bridge logs realmlistreq",
          "[integration][legacy_bnetd][realm_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_realm_dispatch_try(&marker,
                                           "realmlistreq") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(sink.records[0].module == "v3_realm_dispatch_bridge");
    REQUIRE(sink.records[0].message == "realm dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "realmlistreq");
}

TEST_CASE("realm dispatch bridge logs realmlistreq110",
          "[integration][legacy_bnetd][realm_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_realm_dispatch_try(&marker,
                                           "realmlistreq110") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "realmlistreq110");
}

TEST_CASE("realm dispatch bridge logs realmjoinreq109",
          "[integration][legacy_bnetd][realm_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_realm_dispatch_try(&marker,
                                           "realmjoinreq109") == 0);
    REQUIRE(sink.records.size() == 1u);
    REQUIRE(field_value(sink.records[0], "op") == "realmjoinreq109");
}

TEST_CASE("realm dispatch bridge null op normalised",
          "[integration][legacy_bnetd][realm_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_realm_dispatch_try(&marker, nullptr) == 0);
    REQUIRE(sink.records.size() == 1u);
