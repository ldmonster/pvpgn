...
// Unit tests for `pvpgn_v3_send_clanmember_remove_reply` (SID 0x78).
...
#include "integration/legacy_bnetd/send_clanmember_remove_reply_bridge.hpp"
...
TEST_CASE("send_clanmember_remove_reply returns 0 with no send handler installed",
          "[integration][legacy_bnetd][send_clanmember_remove_reply_bridge]") {
    auto* saved = ila::get_send_packet_handler();
    ila::set_send_packet_handler(nullptr);
    int marker = 0;
    REQUIRE(::pvpgn_v3_send_clanmember_remove_reply(&marker, 0xdeadbeefu, 0x01u) == 0);
    ila::set_send_packet_handler(saved);
}

TEST_CASE("send_clanmember_remove_reply rejects null conn pointer",
          "[integration][legacy_bnetd][send_clanmember_remove_reply_bridge]") {
    ScopedSink scope;
    REQUIRE(::pvpgn_v3_send_clanmember_remove_reply(nullptr, 0u, 0u) == 0);
    REQUIRE(FakeSink::call_count == 0);
}

TEST_CASE("send_clanmember_remove_reply emits 9-byte SID 0x78 wire format",
          "[integration][legacy_bnetd][send_clanmember_remove_reply_bridge]") {
    ScopedSink scope;
    int marker = 0;
    REQUIRE(::pvpgn_v3_send_clanmember_remove_reply(&marker, 0x12345678u, 0x01u) == 1);
    REQUIRE(FakeSink::last_conn == &marker);

    REQUIRE(FakeSink::last_bytes.size() == 9u);
    REQUIRE(FakeSink::last_bytes[0] == 0xffu);
    REQUIRE(FakeSink::last_bytes[1] == 0x78u);
...
TEST_CASE("send_clanmember_remove_reply propagates handler return values",
          "[integration][legacy_bnetd][send_clanmember_remove_reply_bridge]") {
    ScopedSink scope;
    int marker = 0;
    FakeSink::return_value = 0;
    REQUIRE(::pvpgn_v3_send_clanmember_remove_reply(&marker, 0u, 0u) == 0);
    FakeSink::return_value = -1;
    REQUIRE(::pvpgn_v3_send_clanmember_remove_reply(&marker, 0u, 0u) == -1);
    FakeSink::return_value = 1;
    REQUIRE(::pvpgn_v3_send_clanmember_remove_reply(&marker, 0u, 0u) == 1);
}
