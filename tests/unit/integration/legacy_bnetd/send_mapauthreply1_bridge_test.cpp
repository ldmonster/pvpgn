...
// Unit tests for `pvpgn_v3_send_mapauthreply1`.
//
// `pvpgn_v3_send_mapauthreply1` builds SERVER_MAPAUTHREPLY1
// (SID_MAPAUTH1, 0x32) bytes via the v3 codec and dispatches through the
...
TEST_CASE("send_mapauthreply1 returns 0 with no send handler installed",
          "[integration][legacy_bnetd][send_mapauthreply1_bridge]") {
    auto* saved = ila::get_send_packet_handler();
    ila::set_send_packet_handler(nullptr);

    int marker = 0;
    REQUIRE(::pvpgn_v3_send_mapauthreply1(&marker, 0u) == 0);

    ila::set_send_packet_handler(saved);
}

TEST_CASE("send_mapauthreply1 rejects null conn pointer",
          "[integration][legacy_bnetd][send_mapauthreply1_bridge]") {
    ScopedSink scope;
    REQUIRE(::pvpgn_v3_send_mapauthreply1(nullptr, 0u) == 0);
    REQUIRE(FakeSink::call_count == 0);
}

TEST_CASE("send_mapauthreply1 emits correct wire bytes (response=0, denied)",
          "[integration][legacy_bnetd][send_mapauthreply1_bridge]") {
    ScopedSink scope;
    int marker = 42;

    REQUIRE(::pvpgn_v3_send_mapauthreply1(&marker, 0u) == 1);
    REQUIRE(FakeSink::call_count == 1);
    REQUIRE(FakeSink::last_conn == &marker);

    // Wire layout: FF 32 08 00 | 00 00 00 00
    //   FF = BNET magic, 0x32 = SID_MAPAUTH1, 0x0008 = length, response=0
    REQUIRE(FakeSink::last_bytes.size() == 8u);
    REQUIRE(FakeSink::last_bytes[0] == 0xFF);  // BNET magic
    REQUIRE(FakeSink::last_bytes[1] == 0x32);  // SID_MAPAUTH1
...
TEST_CASE("send_mapauthreply1 emits correct wire bytes (response=1, normal OK)",
          "[integration][legacy_bnetd][send_mapauthreply1_bridge]") {
    ScopedSink scope;
    int marker = 42;

    REQUIRE(::pvpgn_v3_send_mapauthreply1(&marker, 1u) == 1);
    REQUIRE(FakeSink::last_bytes.size() == 8u);
    REQUIRE(FakeSink::last_bytes[1] == 0x32);
    // response = 1 (little-endian)
...
TEST_CASE("send_mapauthreply1 propagates handler return value",
          "[integration][legacy_bnetd][send_mapauthreply1_bridge]") {
    ScopedSink scope;
    int marker = 0;
