...
// Unit tests for `pvpgn_v3_send_startgame1ack`.
//
// `pvpgn_v3_send_startgame1ack` builds SERVER_STARTGAME1_ACK
// (SID_STARTGAME1, 0x08) bytes via the v3 codec and dispatches through the
...
#include "integration/legacy_bnetd/send_startgame1ack_bridge.hpp"
...
TEST_CASE("send_startgame1ack returns 0 with no send handler installed",
          "[integration][legacy_bnetd][send_startgame1ack_bridge]") {
    auto* saved = ila::get_send_packet_handler();
    ila::set_send_packet_handler(nullptr);

    int marker = 0;
    REQUIRE(::pvpgn_v3_send_startgame1ack(&marker, 0u) == 0);

    ila::set_send_packet_handler(saved);
}

TEST_CASE("send_startgame1ack rejects null conn pointer",
          "[integration][legacy_bnetd][send_startgame1ack_bridge]") {
    ScopedSink scope;
    REQUIRE(::pvpgn_v3_send_startgame1ack(nullptr, 0u) == 0);
    REQUIRE(FakeSink::call_count == 0);
}

TEST_CASE("send_startgame1ack emits correct wire bytes (reply=0, OK)",
          "[integration][legacy_bnetd][send_startgame1ack_bridge]") {
    ScopedSink scope;
    int marker = 0;

    REQUIRE(::pvpgn_v3_send_startgame1ack(&marker, 0u) == 1);
    ...
    // header: FF 08 08 00  (0x08 = 8)
    REQUIRE(FakeSink::last_bytes[0] == 0xFFu);
    REQUIRE(FakeSink::last_bytes[1] == 0x08u);
...
TEST_CASE("send_startgame1ack emits correct wire bytes (reply=1, NO)",
          "[integration][legacy_bnetd][send_startgame1ack_bridge]") {
    ScopedSink scope;
    int marker = 0;

    REQUIRE(::pvpgn_v3_send_startgame1ack(&marker, 1u) == 1);
    REQUIRE(FakeSink::last_bytes.size() == 8u);
...
TEST_CASE("send_startgame1ack propagates handler return value",
          "[integration][legacy_bnetd][send_startgame1ack_bridge]") {
    ScopedSink scope;
    int marker = 0;
