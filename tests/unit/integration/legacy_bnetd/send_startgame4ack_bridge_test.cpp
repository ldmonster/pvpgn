...
// Unit tests for `pvpgn_v3_send_startgame4ack`.
//
// `pvpgn_v3_send_startgame4ack` builds SERVER_STARTGAME4_ACK
// (SID_STARTADVEX4, 0x1C) bytes via the v3 codec and dispatches through the
...
TEST_CASE("send_startgame4ack returns 0 with no send handler installed",
          "[integration][legacy_bnetd][send_startgame4ack_bridge]") {
    auto* saved = ila::get_send_packet_handler();
    ila::set_send_packet_handler(nullptr);

    int marker = 0;
    REQUIRE(::pvpgn_v3_send_startgame4ack(&marker, 0u) == 0);

    ila::set_send_packet_handler(saved);
}

TEST_CASE("send_startgame4ack rejects null conn pointer",
          "[integration][legacy_bnetd][send_startgame4ack_bridge]") {
    ScopedSink scope;
    REQUIRE(::pvpgn_v3_send_startgame4ack(nullptr, 0u) == 0);
    REQUIRE(FakeSink::call_count == 0);
}

TEST_CASE("send_startgame4ack emits correct wire bytes (reply=0, OK)",
          "[integration][legacy_bnetd][send_startgame4ack_bridge]") {
    ScopedSink scope;
    int marker = 0;

    REQUIRE(::pvpgn_v3_send_startgame4ack(&marker, 0u) == 1);
    ...
    // header: FF 1C 08 00  (0x1C = SID_STARTADVEX4)
    REQUIRE(FakeSink::last_bytes[0] == 0xFFu);
    REQUIRE(FakeSink::last_bytes[1] == 0x1Cu);
...
TEST_CASE("send_startgame4ack emits correct wire bytes (reply=1, NO)",
          "[integration][legacy_bnetd][send_startgame4ack_bridge]") {
    ScopedSink scope;
    int marker = 0;

    REQUIRE(::pvpgn_v3_send_startgame4ack(&marker, 1u) == 1);
    REQUIRE(FakeSink::last_bytes.size() == 8u);
...
TEST_CASE("send_startgame4ack propagates handler return value",
          "[integration][legacy_bnetd][send_startgame4ack_bridge]") {
    ScopedSink scope;
    int marker = 0;
