...
#include "integration/legacy_bnetd/telemetry_dispatch_bridge.hpp"
...
TEST_CASE("telemetry dispatch bridge null conn no-op",
...
TEST_CASE("telemetry dispatch bridge logs udpok",
          "[integration][legacy_bnetd][telemetry_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    REQUIRE(sink.records[0].module
            == "v3_telemetry_dispatch_bridge");
    REQUIRE(sink.records[0].message
            == "telemetry dispatch observed");
    REQUIRE(field_value(sink.records[0], "op") == "udpok");
}

TEST_CASE("telemetry dispatch bridge logs fileinforeq",
          "[integration][legacy_bnetd][telemetry_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("telemetry dispatch bridge logs extrawork",
          "[integration][legacy_bnetd][telemetry_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("telemetry dispatch bridge logs crashdump",
          "[integration][legacy_bnetd][telemetry_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("telemetry dispatch bridge null op normalised",
          "[integration][legacy_bnetd][telemetry_dispatch_bridge]") {
    int marker = 0;
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
