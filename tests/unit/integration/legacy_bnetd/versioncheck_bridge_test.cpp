...
// Unit tests for the versioncheck observation bridge.
...
TEST_CASE("versioncheck load bridge logs file",
    ...
    REQUIRE(r.message == "versioncheck load observed");
    REQUIRE(field_value(r, "file") == "/etc/pvpgn/versioncheck.json");
}

TEST_CASE("versioncheck load bridge null filename",
          "[integration][legacy_bnetd][versioncheck_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
...
TEST_CASE("versioncheck unload bridge logs",
          "[integration][legacy_bnetd][versioncheck_bridge]") {
    RecordingLogger sink;
    ila::BridgeLoggerOverride guard(sink);
    ...
    const auto& r = sink.records.front();
    REQUIRE(r.module  == "v3_versioncheck_bridge");
    REQUIRE(r.message == "versioncheck unload observed");
