...
// Unit tests for the d2cs client-packet-dispatcher observation bridge (R235).
...
TEST_CASE("d2cs client-packet bridge logs scalars",
          "[integration][legacy_d2cs][handle_d2cs_packet_bridge]") {
    RecordingLogger sink;
    ilc::BridgeLoggerOverride guard(sink);
    REQUIRE(::pvpgn_v3_d2cs_handle_d2cs_packet_try(9, 0x14u, 64u) == 0);
    REQUIRE(sink.records.size() == 1u);
    const auto& r = sink.records.front();
    REQUIRE(r.level   == pvpgn::core::LogLevel::Trace);
    REQUIRE(r.module  == "v3_d2cs_handle_d2cs_bridge");
    REQUIRE(r.message == "client packet dispatched");
    REQUIRE(r.fields.size() == 3u);
    REQUIRE(field_value(r, "sd")          == "9");
    REQUIRE(field_value(r, "packet_type") == "20");
    REQUIRE(field_value(r, "packet_size") == "64");
}

TEST_CASE("d2cs client-packet bridge override clears on guard destruction",
          "[integration][legacy_d2cs][handle_d2cs_packet_bridge]") {
    RecordingLogger sink;
    {
        ilc::BridgeLoggerOverride guard(sink);
        REQUIRE(::pvpgn_v3_d2cs_handle_d2cs_packet_try(1, 1u, 1u) == 0);
    }
    const std::size_t before = sink.records.size();
    REQUIRE(::pvpgn_v3_d2cs_handle_d2cs_packet_try(2, 2u, 2u) == 0);
