...
// Unit tests for the d2cs d2gs-packet-dispatcher observation bridge (R235).
...
#include "integration/legacy_d2cs/handle_d2gs_packet_bridge.hpp"
...
TEST_CASE("d2cs d2gs-packet bridge logs scalars",
    ...
    REQUIRE(::pvpgn_v3_d2cs_handle_d2gs_packet_try(33, 0x42u, 256u) == 0);
...
TEST_CASE("d2cs d2gs-packet bridge override clears on guard destruction",
    ...
    {
        ilc::BridgeLoggerOverride guard(sink);
        REQUIRE(::pvpgn_v3_d2cs_handle_d2gs_packet_try(1, 1u, 1u) == 0);
    }
    const std::size_t before = sink.records.size();
    REQUIRE(::pvpgn_v3_d2cs_handle_d2gs_packet_try(2, 2u, 2u) == 0);
