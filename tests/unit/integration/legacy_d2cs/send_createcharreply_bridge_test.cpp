...
// Unit tests for `pvpgn_v3_d2cs_send_createcharreply`
// (D2CS_CLIENT_CREATECHARREPLY 0x02).

#include <catch2/catch_test_macros.hpp>

#include <vector>

#include "app/d2cs/legacy_d2cs_bridges/send_createcharreply_bridge.hpp"
...
TEST_CASE("d2cs send_createcharreply returns 0 with no handler",
          "[integration][legacy_d2cs][send_createcharreply_bridge]") {
    auto* saved = ild::get_send_packet_handler();
    ild::set_send_packet_handler(nullptr);
    int marker = 0;
    REQUIRE(::pvpgn_v3_d2cs_send_createcharreply(&marker, 0u) == 0);
    ild::set_send_packet_handler(saved);
}

TEST_CASE("d2cs send_createcharreply rejects null conn",
          "[integration][legacy_d2cs][send_createcharreply_bridge]") {
    ScopedSink scope;
    REQUIRE(::pvpgn_v3_d2cs_send_createcharreply(nullptr, 0u) == 0);
    REQUIRE(FakeSink::call_count == 0);
}

TEST_CASE("d2cs send_createcharreply emits 7-byte type 0x02 wire",
          "[integration][legacy_d2cs][send_createcharreply_bridge]") {
    ScopedSink scope;
    int marker = 0;
    REQUIRE(::pvpgn_v3_d2cs_send_createcharreply(&marker, 0x14u) == 1);
    ...
    REQUIRE(FakeSink::last_bytes[2] == 0x02u);  // type D2CS_CLIENT_CREATECHARREPLY
    REQUIRE(FakeSink::last_bytes[3] == 0x14u);  // reply LE (ALREADY_EXISTS)
    REQUIRE(FakeSink::last_bytes[4] == 0x00u);
    REQUIRE(FakeSink::last_bytes[5] == 0x00u);
    REQUIRE(FakeSink::last_bytes[6] == 0x00u);
}

TEST_CASE("d2cs send_createcharreply propagates handler return values",
          "[integration][legacy_d2cs][send_createcharreply_bridge]") {
    ScopedSink scope;
    int marker = 0;
    ...
    REQUIRE(::pvpgn_v3_d2cs_send_createcharreply(&marker, 0u) == -1);
    FakeSink::return_value = 1;
    REQUIRE(::pvpgn_v3_d2cs_send_createcharreply(&marker, 0u) == 1);
}
