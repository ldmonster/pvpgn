...
// Unit tests for the d2cs server-process observation bridge (R234).
...
TEST_CASE("d2cs server_process bridge logs at info",
    ...
    REQUIRE(r.module  == "v3_d2cs_server_bridge");
    REQUIRE(r.message == "server_process entry observed");
    REQUIRE(r.fields.empty());
}

TEST_CASE("d2cs server_process bridge override clears on guard destruction",
          "[integration][legacy_d2cs][server_bridge]") {
    RecordingLogger sink;
    {
        ilc::BridgeLoggerOverride guard(sink);
