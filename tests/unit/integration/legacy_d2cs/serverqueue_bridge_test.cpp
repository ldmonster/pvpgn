...
// Unit tests for the d2cs server-queue lifecycle observation bridge (R236).
...
TEST_CASE("d2cs sqlist create bridge logs at debug",
    ...
    REQUIRE(r.message == "sqlist create observed");
}

TEST_CASE("d2cs sqlist destroy bridge logs at debug",
          "[integration][legacy_d2cs][serverqueue_bridge]") {
    RecordingLogger sink;
    ilc::BridgeLoggerOverride guard(sink);
    ...
    const auto& r = sink.records.front();
    REQUIRE(r.module  == "v3_d2cs_serverqueue_bridge");
    REQUIRE(r.message == "sqlist destroy observed");
}

TEST_CASE("d2cs sqlist bridge override clears on guard destruction",
          "[integration][legacy_d2cs][serverqueue_bridge]") {
    RecordingLogger sink;
    {
        ilc::BridgeLoggerOverride guard(sink);
